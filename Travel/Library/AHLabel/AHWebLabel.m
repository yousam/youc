...
@interface AHWebLabelButton : UIButton
{
@private
    int componentIndex;
    NSURL *url;
...
@implementation AHWebLabelButton
...
@interface AHWebLabelComponent : NSObject
{
@private
    NSString *text;
    NSString *tagLabel;
...
@implementation AHWebLabelComponent
@synthesize text;
@synthesize tagLabel;
...
- (id)initWithFrame:(CGRect)_frame {
    ...
    if (self) {
        ...
        //self._text = @"";
        [self setText:@""];
        _textAlignment = AHTextAlignmentLeft;
        _lineBreakMode = AHTextLineBreakModeWordWrapping;
        _lineSpacing = 3;
...
- (void)render
{
    ...
    for (AHWebLabelComponent *component in self._textComponents)
    {
    ...
    if (currentSelectedButtonComponentIndex==-1)
    {
        ...
        for (AHWebLabelComponent *linkableComponents in links)
        {
            float height = 0.0;
            ...
            for (CFIndex i=0; i<CFArrayGetCount(frameLines); i++)
            {
                ...
                if ( (linkableComponents.position<lineRange.location && linkableComponents.position+linkableComponents.text.length>lineRange.location) || (linkableComponents.position>=lineRange.location && linkableComponents.position<lineRange.location+lineRange.length))
                {
                    ...
                    AHWebLabelButton *button = [[AHWebLabelButton alloc] initWithFrame:CGRectMake(primaryOffset, height, button_width, ascent+descent)];
...
{
    AHWebLabelButton *button = (AHWebLabelButton*)sender;
...
- (void)onButtonPressed:(id)sender
{
    ...
    if ([delegate respondsToSelector:@selector(ahLabel:didSelectLinkWithURL:)])
    {
        [delegate ahLabel:self didSelectLinkWithURL:button.url];
    }
}
...
//参考： http://forums.macrumors.com/showthread.php?t=925312
    ...
    while (![scanner isAtEnd])
    {
        ...
        if ([text rangeOfString:@"</"].location==0)
        {
            ...
            if (position!=NSNotFound)
            {
                ...
                {
                    AHWebLabelComponent *component = [components objectAtIndex:i];
        ...
        else
        {
            ...
            //NSLog(@"%@", attributes);
            AHWebLabelComponent *component = [AHWebLabelComponent componentWithString:nil tag:tag attributes:attributes];
        ...
        if([valid_tags containsObject:tag] == NO)
        {
            ...
            if (position!=NSNotFound)
            {
                ...
                if (isEnd)
                {
                    ...
                    [components addObject:[AHWebLabelComponent componentWithString:text2 tag:tag attributes:lastAttributes]];
                }
                else
                {
                    // 外标签
                    //NSLog(@">>>>>>> normal: %@ %@", text2, lastAttributes);
                    [components addObject:[AHWebLabelComponent componentWithString:text2 tag:nil attributes:lastAttributes]];
                }
